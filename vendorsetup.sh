...
# vendor_boot
export FOX_RECOVERY_VENDOR_BOOT_PARTITION=true
...
git apply --reverse --check ../../device/infinix/X6531/patches/TWRP-Samsung-JDM-Vibration.patch || REVERSE=$?
cd ../../
...
if [ -f "$TFILE" ];then
    ...
    cd bootable/recovery
    git apply ../../device/infinix/X6531/patches/TWRP-Samsung-JDM-Vibration.patch || RET=$?
    cd ../../
    if [ $RET -ne 0 ];then
